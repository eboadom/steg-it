...
let get_available_bits = (image, bits_rate=4) => {
    // height * width * rgba components * bits rate per component
    let available_bits = (image.bitmap.height * image.bitmap.width * 4 * bits_rate);
...
let get_message_from_pixels = (image, length_message, bits_rate = 4) => {
    let recovered_message = [];
    let index_message = 0;
    let width_needed_to_code = Math.ceil(length_message / (image.bitmap.width * 4 * 4));
    console.log("Bits rate en get from pixels is "+bits_rate);
    for (var index_row = 0, width = width_needed_to_code; index_row < width; index_row++) {
        for (var index_col = 0, height = image.bitmap.height; index_col < height; index_col++) {
            ["r", "g", "b", "a"]
...
exports.test = (path_image) => {
    ...
    // Load the image with Promise
        ...
        .then(function (image) {
            ...
            let message_recuperado = get_message_from_pixels(image_hacked, message.length, bits_rate);
            console.log("-- Mensaje recuperado (con cabecera de tamaño)=> " + message_recuperado);
